...
- (void)viewDidLoad
{
	...
	framedepth = 0;
	framergb = 0;
	frameacc[0]=frameacc[1]=frameacc[2]=0;
	framerot[0]=framerot[1]=framerot[2]=0;
	framexy[0]=self.view.frame.size.width;
	framexy[1]=self.view.frame.size.height;
}
                                   ...
                                   colorFrame:(STColorFrame *)colorFrame
{
	...
	BOOL isDir = NO;
	NSString *filePath =  [documentsDirectory stringByAppendingPathComponent:@"SomeFile1.txt"];
	const char *path = [filePath UTF8String];
	FILE* fp = fopen(path, "ab");
	fwrite(&currentMaxRotX, 1, 8, fp);
	fwrite(&currentMaxRotY, 1, 8, fp);
	fwrite(&currentMaxRotZ, 1, 8, fp);
	fwrite(&currentMaxAccelX, 1, 8, fp);
	fwrite(&currentMaxAccelY, 1, 8, fp);
	fwrite(&currentMaxAccelZ, 1, 8, fp);
	fwrite(framedepth, 1, 1 * 2 * framexy[1] * framexy[0], fp);
	fwrite(framergb, 1, 1 * 3 * framexy[1] * framexy[0], fp);
